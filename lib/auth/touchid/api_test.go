...
func TestLogin_findsCorrectCredential(t *testing.T) {
	...
	*touchid.Native = fake
	...
	// Register a few credentials for a second RPID, if everything is correct this
	// shouldn't interfere with the test, despite happening last.
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, actualUser, err := touchid.Login(origin, test.user, &wanlib.CredentialAssertion{
				Response: protocol.PublicKeyCredentialRequestOptions{
					Challenge:          []byte{1, 2, 3, 4, 5}, // arbitrary
					RelyingPartyID:     web.Config.RPID,
					AllowedCredentials: allowedCreds,
					UserVerification:   "discouraged", // not important here
				},
			})
			require.NoError(t, err, "Login failed")
			if test.user != "" {
				assert.Equal(t, test.user, actualUser, "Login user mismatch")
			}
